...
$.fn.searchRemind=function(options){
	...
	function shareto(sign,pos,the){
		var f= encodeURIComponent('http://join.qq.com/');
		if(pos=='top'){//preach
			var thet = '想加入吗？请登录join.qq.com，查看腾讯最新校园招聘动态。';
			var t = encodeURIComponent(thet);
			var u = encodeURIComponent('http://join.qq.com/');
			var c = encodeURIComponent(t+'\n'+u);
		}else if(pos=='preach'){
			var pp = $(the).parents('.i:first');
			var share_xx = pp.find('.share_xx:first').text();
			var share_sj = pp.find('.share_sj:first').text();
			var share_dz = pp.find('.share_dz:first').text();
			var thet = '腾讯2013校园招聘'+share_xx+'宣讲会于'+share_sj+'在'+share_dz+'举行，敬请关注！';
			var t = encodeURIComponent(thet);
			var u = encodeURIComponent('http://join.qq.com/');
			var c = encodeURIComponent(t+'\n'+u);
		}else if(pos=='hack'){		
			var thet = '腾讯2013年编程马拉松大赛火热报名中！在这里，你可以享受与编程高手PK的乐趣，还可能前往腾讯参加为期两天一夜的决赛，更有机会获得万元奖金与腾讯实习offer！奔跑吧,校园极客！创造自己的极客style！';		
			var t = encodeURIComponent(thet);
			var u = encodeURIComponent(window.location.href);
			var c = encodeURIComponent(thet+'\n'+u);
		}
		else{
			var thet = '腾讯2013校园招聘已经启动，推荐校园招聘岗位'+$('#posttitle').val()+'，大家赶快来投递简历吧！';
			var t = encodeURIComponent(thet);
			var u = encodeURIComponent('http://join.qq.com/');
			var c = encodeURIComponent(thet+'\n'+u);
		}
